    ...
    <div>
      ...
      <ul>
        <li v-for="std in student" :key="std.code">
          ...
          <v-btn color="warning" @click="edit(std.code)">EDIT</v-btn>
          <v-btn color="red" @click="del(std.code)">DEL</v-btn>
        </li>
      </ul>
    </div>
    <nuxt-link to="form_list_card">
      <v-btn color="warning">Card List</v-btn>
    </nuxt-link>
    <nuxt-link to="add_std">
